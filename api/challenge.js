...
import { randomHex,
         randomToken }       from "./_lib/crypto.js";
import { checkRedisRateLimit,
         checkBan }          from "./_lib/redis.js";
...
export default async function handler(req, res) {
  ...
  // ── Layer 5: Rate limit ───────────────────────────────────────────────
  const { challenge } = CONFIG.rateLimit;
  ...
  const { expirySeconds } = CONFIG.challenge;
  ...
  return res.status(200)
            ...
            .json({ challenge_id, nonce, timestamp });
}
